...
/**
 * Aggregate Room database implementing every Proton Core storage interface required for
 * account, auth, user, address, key, crypto and settings persistence.
 *
 * Unlike the Proton Calendar app, this starts fresh at version 1 with destructive
 * migration: FrontWidget never ships an older schema, so there is no history to migrate.
 */
@Database(
    entities = [
        AccountEntity::class,
...
abstract class AppDatabase :
    ...
    companion object {
        const val name = "proton.frontwidget.db"
        const val version = 1
